...
\textbf{1.503 números}. \textbf{Cuando se abrió el tramo faltaban treinta y uno}: el del 977 al
990 (fines de septiembre a fines de diciembre de 1923), el del 1.037 al 1.042 (21 de noviembre a
26 de diciembre de 1924), el 1.095 (1 de enero de 1926), el 1.146 (24 de diciembre de 1926), el
1.305 (10 de enero de 1930), el 1.312, 1.318, 1.320, 1.323 y 1.327 (febrero a junio de 1930), el
1.356 (2 de enero de 1931), y el 1.512 y el 1.564, del último viernes de 1933 y de 1934. \textbf{Las
lecturas posteriores de 1923, 1924 y 1926 enteros recuperaron veintiuno}, y el índice de los
\textit{Releases} de \texttt{boletines-salta}, relevado el 24 de septiembre de 2026, confirma que
están (lámina \ref{fig:boletin}). \textbf{Ese mismo índice encontró cinco que este apéndice no
declaraba}: \textbf{del 2.028 al 2.032}, de fines de 1943, que no están en el \textit{Release}.
Cuatro de ellas ---\textbf{la 2.028 (12 de noviembre), la 2.029 (19 de noviembre), la 2.031 (3 de
diciembre) y la 2.032 (10 de diciembre)}--- están en la copia de trabajo del autor y se leyeron
enteras al cierre, 46, 32, 33 y 32 páginas. Traen dos piezas del departamento o de su sistema de
ríos: la defensa del Mojotoro de 1943 y el decreto que nombra comisario de La Caldera en la
vacante de uno que murió en el cargo, que el capítulo \ref{cap:siglo} incorpora. \textbf{Ninguna
trae el acto que dispuso la intervención de la comuna} que el capítulo \ref{cap:opacidad} no
logra fechar. \textbf{Sólo la 2.030, del 26 de noviembre, no se obtuvo}: se cuenta como no leída,
y el apéndice \ref{cap:pedidos} la pide. \textbf{Quedan así seis números ausentes ---1.095,
1.305, 1.356, 1.512, 1.564 y 2.030--- y cinco presentes pero en blanco ---1.312, 1.318, 1.320,
1.323 y 1.327---: 1.492 leídos}. Del 1.512 y el 1.564, además, \textbf{el índice del propio repositorio no registra} el
número: la lista de 1933 termina en el Nº 1.511, del 22 de diciembre, y la de 1934 empieza en el
1.513, del 5 de enero; la de 1934 termina en el 1.563, del 21 de diciembre, y la de 1935 empieza en
el 1.565, del 4 de enero, y no se sabe si se publicaron y no se digitalizaron o si la numeración
saltó. \textbf{Los ausentes tienen, además, una estación}: cuatro de los cinco que este apéndice ya
declaraba ---el 1.095, el 1.356, el 1.512 y el 1.564--- son números de la última semana de
diciembre o la primera de enero, y la de 1943 cae en las semanas previas al cierre del
año. El mismo índice cubre 1910 a 1943: 1908, 1909 y 1944 en adelante no tienen \textit{Release} en
ese repositorio, y ahí el recuento sigue siendo el de la lectura. A eso se suman lagunas de página dentro de ediciones leídas: las páginas
...
demandadas acompañaron en cumplimiento de la condena. De este incidente provienen siete de las cuarenta y una láminas de este volumen.
...
Veinte de las cuarenta y una láminas son de elaboración propia: \textbf{once mapas}
...
\ref{fig:otbn}, \ref{fig:agua} y \ref{fig:fracciones}--- y \textbf{nueve gráficos} ---\ref{fig:parajesnom}, \ref{fig:caudales},
\ref{fig:formacioncomparacion}, \ref{fig:padron}, \ref{fig:cargos}, \ref{fig:coparticipacion}, \ref{fig:defensas}, \ref{fig:boletin} y \ref{fig:votado}---. Todas salen del \textbf{repositorio cartográfico \texttt{mapas\_caldera}}
...
figuras resultantes} y \textbf{las capas propias}: las tres tablas de cargos, la planilla de
ribera, las tablas de montos de \texttt{datos/fiscal/}, el índice de ediciones de
\texttt{datos/boletin/} y las tablas censales de \texttt{datos/censo\_tablas/}, que son
transcripción o procesamiento de este trabajo y de nadie más. \textbf{Las capas de terceros
...
\item \texttt{scripts/tabla\_parajes.py} --- lámina \ref{fig:parajesnom}; la tabla de trece
nóminas que arma este script ---la decimotercera, de 1948, se sumó al cierre--- es además \textbf{la fuente única} de la lámina siguiente.
...
publicación y su fecha de consulta. \textbf{Sólo esa lámina}: la \ref{fig:catastro} usa la misma planilla, pero tiene su propio script,
que sigue.
\item \texttt{scripts/mapa\_catastro\_ribera.py} --- lámina \ref{fig:catastro}, sobre la misma
planilla de ribera y sobre \texttt{datos/ribera/actos\_por\_parcela.csv}, que transcribe la tabla del
capítulo \ref{cap:ribera} parcela por parcela. Al rehacerla se corrigió un error de la versión
anterior: pintaba como línea de ribera la matrícula 1534, que sólo tiene una comisión designada.
...
\item \texttt{scripts/grafico\_defensas.py} --- lámina \ref{fig:defensas}, sobre \texttt{datos/fiscal/defensas\_1947.csv}, que transcribe los nueve ríos del Ítem 6 de la Ley 834 con su monto, y \texttt{defensas\_1949.csv}, con los cuatro renglones del departamento en el Ítem 6 de 1949 y el resto del ítem en uno solo.
\item \texttt{scripts/mapa\_fracciones.py} --- lámina \ref{fig:fracciones}, sobre \texttt{datos/censo\_tablas/fracciones\_2022.csv}, que reúne en una sola tabla los números del Censo 2022 que este libro usa por fracción.
\item \texttt{scripts/grafico\_boletin.py} --- lámina \ref{fig:boletin}, sobre \texttt{datos/boletin/}: el índice de ediciones presentes en los \textit{Releases} 1910--1943 de \texttt{boletines-salta}, relevado el 24 de septiembre de 2026, y el estado de lectura de cada tramo según este apéndice.
\item \texttt{scripts/grafico\_votado.py} --- lámina \ref{fig:votado}, sobre \texttt{datos/fiscal/votado\_ejecutado.csv}, que cita para cada fila el lugar del libro de donde sale.
\item \texttt{scripts/grafico\_padron.py} --- lámina \ref{fig:padron}, sobre \texttt{datos/electoral/}: los electores por municipio de cada elección y las tres referencias censales.
\item \texttt{scripts/grafico\_formacion.py} --- lámina \ref{fig:formacioncomparacion}, sobre \texttt{datos/censo\_tablas/formacion\_2022.csv}. Al rehacerla se corrigió un rótulo: la versión anterior llamaba «El Durazno» al radio 0209, y el capítulo \ref{cap:poblacion} establece que la coordenada no permite decir si El Durazno está en el 0208 o en el 0209.
\item \textbf{Y tres láminas propias no tienen todavía su script en el repositorio}, porque
les faltan los datos de entrada: el mapa del ordenamiento de bosques (\ref{fig:otbn}), que
necesita el ráster del mapa final del informe técnico de la Ley 8483; el de formación por radio
censal (\ref{fig:formacionradios}), que necesita la tabla de los veintiocho radios salida de
Redatam; y el gráfico de caudales (\ref{fig:caudales}), que necesita las series mensuales del
Sistema Nacional de Información Hídrica. Las fuentes están declaradas en cada epígrafe, de modo
que son verificables; \textbf{pero no son reproducibles paso a paso hasta que esos datos y su
script se incorporen}. Se declara aquí porque es exactamente la diferencia que este apéndice
acaba de reclamar para las otras diecisiete.
\end{itemize}
...
\textbf{De modo que el estado de las veinte láminas propias, al cierre de este volumen, es
éste}: las veinte son \textbf{verificables} ---cada epígrafe declara con qué datos se hizo, y
esos datos tienen origen y fecha---; \textbf{diecisiete son además reproducibles} ---su script
está publicado, sus capas se bajan del \textit{Release} y la cadena se probó completa---; y
\textbf{tres no lo son todavía}, porque su script no está. \textbf{Verificable no es lo
...
lo prueba. Ninguna de las cuarenta y una láminas del volumen pasó por un modelo generativo, ni
