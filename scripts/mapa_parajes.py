...
for ax, e in ((axA, ext), (axB, extB)):
    ...
    for k in (1, 3, 8):
        sel = hit[hit.n.isin([k, k + 1] if k != 3 else [3, 4, 5, 6, 7])]
        if len(sel):
...
leg = [Patch(fc=COL[8], ec='#6f665a', label='Nombre presente en ocho o nueve de las nueve nóminas'),
       Patch(fc=COL[3], ec='#6f665a', label='Presente en tres a siete'),
...
axL.text(0, 0.16, 'El color no mide superficie ni dominio: mide en cuántas de las nueve\n'
