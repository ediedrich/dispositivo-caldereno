...
\textbf{La imagen de la portada} es una vista aérea del valle del río La Caldera ---el río,
la Ruta 9 sobre la margen este y la ladera de yungas--- con una trama de amanzanamiento
superpuesta. Está fuera del índice de láminas y por eso no lleva epígrafe de procedencia;
se deja dicho aquí que \textbf{la procedencia de la fotografía de base y la autoría de la
trama no quedaron asentadas} al cerrarse este manuscrito, y que es la única imagen del
volumen en esa situación.

Compuesto en Charter. Composición del autor.\par}
\vspace{2em}
\cleardoublepage

\input{cap/00-advertencia}
\tableofcontents
\cleardoublepage
\listoffigures
\mainmatter
...
\input{cap/09-defensas}
\input{cap/10-expropiacion}
\input{cap/11-ribera}
\input{cap/12-amparo}
...
\input{cap/13-loteo}
\parte{El municipio}
{Doce mil habitantes, una de cada cinco viviendas sin residentes, una hacienda que no alcanza desde 1908 y las redes que nunca llegaron.}
\input{cap/14-poblacion}
\input{cap/15-hacienda}
\input{cap/16-redes}
\input{cap/17-aguabaja}
\input{cap/18-trabajo}
...
\input{cap/18-politica}
\input{cap/19-resistencias}
\parte{La opacidad}
...
\input{cap/20-opacidad}
\parte{Cierre}
{Qué quedó probado, qué no, y qué falta para probarlo.}
\input{cap/21-ausencias}
\input{cap/21-conclusion}
\parte{Lo que sigue}
...
\input{cap/22-infraestructura}
\input{cap/22-prospectiva}
\input{cap/23-plan}
\appendix
\parte{Apéndices}
...
\input{ape/A-cronologia}
\input{ape/B-matriz}
\input{ape/C-normativa}
\input{ape/D-pedidos}
\input{ape/E-personas}
\input{ape/F-fuentes}
\input{ape/G-propuestas}
\input{ape/H-dominio}
\backmatter
\cleardoublepage
\printindex
